...
Public Class Invoice_Deal
    ...
    Private Sub Invoice_Deal_BeforePrint(sender As Object, e As System.Drawing.Printing.PrintEventArgs) Handles MyBase.BeforePrint
        ...
        Get_InvoiceTableAdapter.Fill(DataSet11.Get_Invoice, CDec(gInv_No), False)
        ...
        Dim dt = Get_InvoiceTableAdapter.GetData(CType(gInv_No, Decimal?), False)
        ir = dt.ToList.First
        XrTableCellSlov.Text = "Словом: " + Slov(ir.Summ_Local)
        If ir.Printed = 0D Then
            Me.Watermark.Text = "ОРИГИНАЛ"
        ElseIf ir.ANUL Then
            Me.Watermark.Text = "АНУЛИРАН"
        Else
            Me.Watermark.Text = "ДУБЛИКАТ"
        End If

        If Not ir.IsModel_NameNull AndAlso ir.Model_Name.ToUpper.Contains("LEXUS") Then
        ...
        If (ir.Service = 5 OrElse ir.Service = 0) AndAlso ir.Prev_Inv_No = 0D Then
            XrTableOrder.Visible = False
            XrTableOrder.TopF = 0
            XrTableOrder.HeightF = 0
        ...
        ElseIf ir.Service = 5 OrElse ir.Service = 0 Then
            XrTableOrder.Rows(2).Visible = False
            XrTableOrder.Rows(3).Visible = False
            'XrTableOrder.TopF = 0
            'XrTableColumnHeader.TopF = XrTableCons.LocationF.Y + XrTableCons.SizeF.Height + XrTableColumnHeader.SizeF.Height / 5
            XrTableColumnHeader.TopF = XrTableOrder.LocationF.Y + XrTableOrder.SizeF.Height + XrTableColumnHeader.SizeF.Height / 5
            ReportHeader.HeightF = XrTableColumnHeader.TopF + XrTableColumnHeader.HeightF
        End If
        ...
        ZInvoice_Rows_PrintTableAdapter1.Fill(DataSet11.zInvoice_Rows_Print, CDec(gInv_No), 0, DirectCast(ShortPrint.Value, Boolean), DirectCast(ShowPArts.Value, Boolean))
    End Sub
    ...
    Private Sub XrTableRow4_BeforePrint(sender As Object, e As Printing.PrintEventArgs) Handles XrTableRow4.BeforePrint
    ...
    Private Sub ReportFooter_BeforePrint(sender As Object, e As Printing.PrintEventArgs) Handles ReportFooter.BeforePrint
        ...
        If ir Is Nothing Then
            Dim dt = Get_InvoiceTableAdapter.GetData(gInv_No, False)
            ir = dt.ToList.First
        End If
