...
Public Class Proforma1
    ...
    Private Sub Invoice_Deal_BeforePrint(sender As Object, e As Drawing.Printing.PrintEventArgs) Handles MyBase.BeforePrint
        If gShowCodes <> 0 Then
            XrTableCellCode.DataBindings.Clear()
            XrTableCellCode.WidthF = 0
            XrTableCellCodeHeader.WidthF = 0
            XrTableCellCodeHeader1.WidthF = 0
        End If

        Print_ProformaTableAdapter1.Connection.ConnectionString = gConnStr
        Print_ProformaTableAdapter1.Fill(DataSet11.Print_Proforma, CDec(gOrderNo), CDec(gService), CDec(gSplit), CDec(gCurr), CDec(gRateType), gRateDate, CDec(gShowAll), CDec(gOrderType), True, False)
