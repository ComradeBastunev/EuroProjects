...
Public Interface ITixim_Reports_Client
...
<ClassInterface(ClassInterfaceType.None)> Public Class Tixim_Reports
    Implements ITixim_Reports_Client

    Function PrintOrder(aConnStr As String, aService As Integer, aOrder_No As Long, aSplit As Integer) As Integer Implements ITixim_Reports_Client.PrintOrder
        PrintOrder = 0
    ...
    Function PrintCheckList(aConnStr As String, aService As Integer, aOrder_No As Long, aSplit As Integer) As Integer Implements ITixim_Reports_Client.PrintCheckList
        PrintCheckList = 0
        gOrderNo = aOrder_No
        gService = aService
        gSplit = aSplit
    ...
    Function PrintInvoice(aConnStr As String, aInvNo As Double) As Integer Implements ITixim_Reports_Client.PrintInvoice
        PrintInvoice = 0
        gInv_No = aInvNo
        gConnStr = aConnStr
        ...
        GetInvTA1.Fill(DataSet11.Get_Invoice, CDec(aInvNo))
        ...
        Else
            ...
            If DataSet11.Get_Invoice.First.Split = 3 Then
                ReportInv.ShortPrint.Value = MsgBox("Съкратен печат?", CType(MsgBoxStyle.YesNo + MsgBoxStyle.Question, Global.Microsoft.VisualBasic.MsgBoxStyle), "Печат фактура") = MsgBoxResult.Yes
                If DataSet11.Get_Invoice.First.Client_No <> 2500 AndAlso CType(ReportInv.ShortPrint.Value, Boolean) Then
                    ReportInv.ShowPArts.Value = MsgBox("Показване части?", CType(MsgBoxStyle.YesNo + MsgBoxStyle.Question, Global.Microsoft.VisualBasic.MsgBoxStyle), "Печат фактура") = MsgBoxResult.Yes
                End If
            Else
    ...
    Function PreviewInvoice(aConnStr As String, aInvNo As Double) As Integer Implements ITixim_Reports_Client.PreviewInvoice
        PreviewInvoice = 0
        gInv_No = aInvNo
        gConnStr = aConnStr
        Dim DataSet11 As New DataSet1
    ...
    Function PrintProforma(aConnStr As String, aService As Integer, aOrder_No As Long, aSplit As Integer, aCurr As Integer, aRateType As Integer, aRateDate As DateTime, aShowAll As Integer, aBankAcc As Integer, aShowCodes As Integer, aOrder_Type As Integer) As Integer Implements ITixim_Reports_Client.PrintProforma
        PrintProforma = 0
        gOrderNo = aOrder_No
        gService = aService
        gSplit = aSplit
