Imports System.Math
Module Module1
    Public gOrderNo As Long
    Public gService As Integer
    Public gSplit As Integer
    Public gConnStr As String
    Public gInv_No As Double
    Public gForClient As Boolean
    Public gCurr As Integer
    Public gRateType As Integer
    Public gRateDate As DateTime
    Public gShowAll As Integer
    Public gBankAcc As Integer
    Public gShowCodes As Integer
    Public gOrderType As Integer
    ...
    Function Slov(ByVal a As Double) As String
        ...
        a = Math.Truncate(a)
        b = a.ToString
        ran = CInt(Math.Truncate((b.Length - 1) / 3) + 1)
        C = ran * 3 - b.Length
        aSlov = ""
        For I = 1 To C
            b = "0" + b
        ...
        For I = 1 To ran
            ...
            'Изчисляване стотици}
            ran_cval = b1 * 100
            cval = CInt(cval + b1 * 100 * Math.Truncate(Math.Pow(1000, ran - I)))
            ...
            cval = CInt(cval + b2 * 10 * Math.Truncate(Math.Pow(1000, ran - I)))
            ...
            cval = CInt(cval + b3 * Math.Truncate(Math.Pow(1000, ran - I)))
            If b2 = 1 Then
                Select Case b3
                    Case 0
                        aSlov = aSlov + "десет "
